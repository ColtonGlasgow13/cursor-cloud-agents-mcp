...
export interface DrainRunEventStreamOptions {
  ...
  maxWaitMs: number;
  maxEvents: number;
  /** Types to keep in `events`. Defaults to all events except transport heartbeats. */
  eventTypes?: string[];
  retentionSeconds?: number | null;
  now?: () => number;
...
export async function drainRunEventStream({
  ...
  const included = eventTypes === undefined ? null : new Set(eventTypes);
  const isIncluded = (type: string): boolean =>
    type !== 'heartbeat' && (included === null || included.has(type));
  ...
  const onEvent = (message: EventSourceMessage): void => {
    ...
    // Filtered events still move the resume cursor so they are not replayed.
  ...
  return { events, lastEventId, statusFromStream, retentionSeconds, closedByServer };
}
